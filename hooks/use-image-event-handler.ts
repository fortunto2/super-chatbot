    ...
    switch (eventData.type) {
      ...
      case 'file':
        ...
        if (eventData.object) {
          ...
          if ((fileObject.type === FileTypeEnum.IMAGE || fileObject.contentType?.startsWith('image/')) && fileObject.url) {
          ...
          // Handle file_id case - need to resolve to URL using FileService
            ...
            console.log('📁 File ID received, resolving to URL:', fileObject.file_id);
            
            // Import FileService dynamically to resolve file_id to URL
            import('@/lib/api').then(async ({ FileService }) => {
              try {
                ...
                if (fileResponse && fileResponse.url) {
                  ...
                  if (isImage) {
                    console.log('📁 ✅ File ID resolved to image URL:', fileResponse.url);
                    ...
                    if (isValid) {
                     ...
                     console.log('📁 ⚠️ File ID resolved to non-image file:', fileResponse.type);
                  }
                } else {
                  console.error('📁 ❌ File ID resolution failed - no URL in response');
                }
              } catch (error) {
        ...
        // Also handle case where file data is directly in eventData  
          ...
          const isImage = eventData.url.match(/\.(jpg|jpeg|png|webp|gif|bmp|svg)$/i);
        ...
        // Handle direct file_id in eventData
          ...
          const fileId = eventData.object?.file_id || (eventData as any).file_id;
          console.log('📁 Direct file ID received, resolving to URL:', fileId);
          ...
          import('@/lib/api').then(async ({ FileService }) => {
            try {
              const fileResponse = await FileService.fileGetById({ id: fileId });
                 ...
                 if (isImage) {
                  console.log('📁 ✅ Direct file ID resolved to image URL:', fileResponse.url);
                  ...
                  if (isValid) {
                   ...
                   console.log('📁 ⚠️ Direct file ID resolved to non-image file:', fileResponse.type);
                }
              } else {
                console.error('📁 ❌ Direct file ID resolution failed - no URL in response');
              }
            } catch (error) {
              console.error('📁 ❌ Failed to resolve direct file ID to URL:', error);
            }
          }).catch(error => {
            console.error('📁 ❌ Failed to import FileService for direct file ID:', error);
          });
        }
        break;
      ...
      case 'task_status':
        ...
        if (eventData.object?.status === 'COMPLETED') {
          console.log('📡 Task completed for project:', eventData.projectId || projectId);
          
          // Start polling check for completed generation
          setTimeout(async () => {
            try {
              console.log('🔍 Checking for completed images after task completion');
              
              const response = await fetch(`/api/project/${eventData.projectId || projectId}`);
              if (!response.ok) {
              ...
              if (imageData?.value && typeof imageData.value === 'object') {
                ...
                console.log('🔍 ✅ Found completed image via polling:', imageUrl);
              ...
              if (fileIdData?.value && typeof fileIdData.value === 'object') {
                ...
                console.log('🔍 Found file_id via polling, resolving:', fileId);
