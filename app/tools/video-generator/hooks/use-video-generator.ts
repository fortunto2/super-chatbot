...
export function useVideoGenerator(): UseVideoGeneratorReturn {
  ...
  const startPolling = useCallback(async (fileId: string) => {
    ...
    console.log('🔄 Starting smart video polling for file:', fileId);
    ...
    try {
      ...
      const result = await pollFileCompletion(fileId, {
        ...
        onProgress: (attempt, elapsed, nextInterval) => {
          ...
          setGenerationStatus(prev => ({
            ...prev,
            message: `Processing video... (attempt ${attempt}, ${Math.round(elapsed / 1000)}s elapsed)`
          }));
        },
      ...
      if (result.success && result.data) {
        console.log('✅ Smart video polling completed successfully:', result.data.url);
        const projectId = result.data.tasks?.[0]?.project_id || undefined;
        handleGenerationSuccess(result.data.url, projectId);
      } else {
        console.error('❌ Smart video polling failed:', result.error);
        handleGenerationError(result.error || 'Video generation timeout after 7 minutes');
      }
      
    } catch (error) {
      console.error('❌ Smart video polling system error:', error);
      handleGenerationError('Failed to start video polling system');
    }
  }, []);
