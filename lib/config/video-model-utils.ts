...
export async function getEnhancedVideoModels(): Promise<EnhancedVideoModel[]> {
  try {
    ...
    const enhancedModels: EnhancedVideoModel[] = apiModels.map(apiModel => {
      const metadata = videoModelsConfig.model_metadata[apiModel.name as keyof typeof videoModelsConfig.model_metadata];
      ...
      if (metadata?.category) {
        ...
        if (apiModel.name.includes('image-to-video') || apiModel.name.includes('veo') || apiModel.name.includes('kling')) {
          category = 'image_to_video';
        } else if (apiModel.name.includes('lip-sync') || apiModel.name.includes('video-to-video')) {
      ...
      return {
        ...
        uiDescription: metadata?.ui_description || `Generated by ${apiModel.source}`,
