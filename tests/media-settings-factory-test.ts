import { getImageGenerationConfig, getVideoGenerationConfig, clearMediaSettingsCache } from '../lib/config/media-settings-factory';
...
async function testMediaSettingsFactory() {
  ...
  try {
    ...
    console.log(`  Image: ${defaultImageModel.name} (${defaultImageModel.type})`);
    console.log(`  Video: ${defaultVideoModel.name} (${defaultVideoModel.type})`);
    
    // Verify default models are appropriate
    const isImageModelAppropriate = defaultImageModel.type === 'text_to_image' || defaultImageModel.type === 'image_to_image';
    const isVideoModelAppropriate = defaultVideoModel.type === 'text_to_video' || defaultVideoModel.type === 'image_to_video';
    ...
    const requiredFields = ['name', 'type', 'source', 'params', 'id', 'description', 'value', 'workflowPath', 'price'];
    
    const imageModelHasAllFields = requiredFields.every(field => field in sampleImageModel);
    const videoModelHasAllFields = requiredFields.every(field => field in sampleVideoModel);
    
    console.log(`✅ Image model has all required fields: ${imageModelHasAllFields}`);
    console.log(`✅ Video model has all required fields: ${videoModelHasAllFields}`);
    
    if (!imageModelHasAllFields) {
      console.log('❌ Missing image model fields:', requiredFields.filter(field => !(field in sampleImageModel)));
    }
    
    if (!videoModelHasAllFields) {
      console.log('❌ Missing video model fields:', requiredFields.filter(field => !(field in sampleVideoModel)));
    }
